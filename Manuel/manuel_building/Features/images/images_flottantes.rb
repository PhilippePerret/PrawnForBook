Prawn4book::Manual::Feature.new do
  ...
  description <<~EOT
    ...
    * **`text_distance`** | Marge avec le texte. Quand l’image est flottante, permet de définir la marge avec le texte. Elle se substitue à margin_left ou margin_right suivant que l’image est flottante à droite ou flottante à gauche. Elle permet de ne pas avoir à modifier les valeurs :margin_left et :margin_right dès qu’on veut essayer l’image à droite puis à gauche. Elle marche de paire avec la propriété `margin_distance`.
    * **`margin_distance`** | Avec une image flottante, distance avec la marge située du côté où flotte l’image (donc avec la marge gauche si l’image flotte à gauche). S’emploie avec `text_distance` et sans `margin_left` et/ou `margin_right`.
